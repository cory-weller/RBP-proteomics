...
plot_pg_counts <- function(DT, output_dir, output_filename) {
  ...
  if (n_samples>50){
    ggsave(plot = p,filename = paste0(output_dir, output_filename),width = 2+n_samples/10,height = 6)
  }else {
...
plot_pg_intensities <- function(DT, output_dir, output_filename, plot_title) {
    ...
    g <- ggplot(DT, aes(x=Sample, y=log10(Intensity))) + 
        ...
        theme_classic() +
        labs(fill = "",x="",y='Log10 Protein Intensity', title=plot_title) +
        theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5)) +
        geom_boxplot(width=0.1) +
        geom_hline(color='#ef8a62', linetype='dashed',  aes(yintercept=median(log10(DT$Intensity), na.rm=T)))
  
    if (n_samples>50){
        ggsave(plot = g,filename = paste0(output_dir, output_filename),width = 2+n_samples/10,height =6)
    }else{
...
get_spearman <- function(DT.original) {
  ...
  dt.corrs <- cor(log2(as.matrix(dt.samples)), use='complete.obs')
...
get_PCs <- function(DT) {
 ...
 cluster_data=cluster_data[which(rowSums(cluster_data)>0),]
  log2_cluster_data=log2(cluster_data+1)
    log2_cluster_data[is.na(log2_cluster_data)] <- 0
...
plot_RNABP_PCs <- function(PCAobj) {
...
plot_hierarchical_cluster <- function(DT, output_dir) {
  ...
  ggsave(g, filename=paste0(output_dir, 'hc_cluster_log2.pdf'), height=20, width=15)
}
...
enrichAll = function(gene_id, all_gene_vector, outdir='.', MSigDb_gmt_dir = NULL, out_prefix = NULL, width=12, height=8, enrich_pvalue=1){
    ...
    tmp_df=kk@result
    setDT(tmp_df)
    tmp_df[, category := NULL]
    tmp_df[, subcategory := NULL]
    tmp_df$group='kegg'
    enrich_res=rbind(enrich_res,tmp_df)
